...
directory as the current working directory.`,
	...
	terminalEmulator: {
		...
		type: 'string',
		default: 'None',
		enum: ['None', 'CMD (Windows)', 'iTerm2', 'Hyper', 'uxterm (Linux)', 'Gnome terminal'],
		order: 3,
	},
...
function run() {
	...
	switch (terminalEmulator) {
		...
		case 'Hyper':
			...
			var apple_script = `do shell script "open -a Hyper ~"
			delay 1
			tell application "System Events" to keystroke "${escape(shell_command)}" & return`
			child.stdin.write(apple_script);
			child.stdin.end();
			break;
		case 'uxterm (Linux)':
