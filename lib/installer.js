 ...
 */
function installPackage(pkgName: string, version: string): Promise<boolean> {
  return new Promise(resolve => {
    logger.info(`Installing ${pkgName}@${version}...`);

    var command = `${atom.packages.getApmPath()} install --production ${pkgName}@${version}`;
    child_process.exec(command, (err, stdout, stderr) => {
      if (err) {
        // Error while installing package. Return false.
        atom.notifications.addFatalError(`Failed to install/upgrade ${pkgName} to version ${version}. You may need to do it manually.`, {
          detail: stderr
        });
        resolve(false);
      } else {
        // Successfully installed package. Return true.
        atom.notifications.addSuccess(`Successfully installed/upgraded ${pkgName} to version ${version}.`, {
...
export default async function install(): Promise<void> {
  ...
  var packageInfo = JSON.parse(fs.readFileSync(path.join(__dirname, '..', 'package.json')).toString());
